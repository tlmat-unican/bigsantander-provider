 ...
 */
package org.eclipse.bigiot.lib.embeddedspark;
...
import org.eclipse.bigiot.lib.configuration.LibConfiguration;
import org.eclipse.bigiot.lib.handlers.AccessRequestHandler;
import org.eclipse.bigiot.lib.handlers.ProxyAccessRequestHandler;
import org.eclipse.bigiot.lib.misc.Constants;
import org.eclipse.bigiot.lib.offering.OfferingDescription;
import org.eclipse.bigiot.lib.offering.RegistrableOfferingDescription;
import org.eclipse.bigiot.lib.security.AccessToken;
import org.eclipse.bigiot.lib.serverwrapper.BigIotHttpResponse;
import org.eclipse.bigiot.lib.serverwrapper.EmbededdedRouteBasedServer;
...
public class EmbeddedSpark extends EmbededdedRouteBasedServer {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(EmbeddedSpark.class);

  public EmbeddedSpark(String domain, int port) {
    this(domain, port, "", ServerOptionsSpark.defaultOptions);
  }

  public EmbeddedSpark(String domain, int port, String baseRoute,
      ServerOptionsSpark serverOptions) {
    super(domain, port);
  ...
  @Override
  public void start() {
      // TODO: This is a temporary solution until SparkJava supports the
      // stream-based secure method
  ...
  private void start(boolean sslServer, String keyStoreLocation,
    ...
    // TODO: It should be at the end of the method
    sparkInitialized = true;

    String baseUrl = (sslServer ? "https://" : "http://") + this.baseDomain
                     + ":" + this.basePort + "/" + this.baseRoute;
    ...
    Spark.get(baseRoute, "application/json", (req, res) -> {
      ...
      for (Map.Entry<String, OfferingDescription> entry : routes.entrySet()) {
        ...
        obj.put("offering id", entry.getValue().getId());
        if (entry.getKey().startsWith(Constants.SAMPLEDATA_ROUTE)) {
          obj.put("sampledata route", baseUrl + "/" + entry.getKey());
        } else if (entry.getKey().startsWith(Constants.METADATA_ROUTE)) {
          obj.put("metadata route", baseUrl + "/" + entry.getKey());
        } else {
          for (int i = 0; i < entry.getValue().getEndpoints().size(); i++) {
            obj
                .put("access route" + i,
                     entry.getValue().getEndpoints().get(0).getUri());
          }
        }
        jsonList.put(obj);
      }

      jsonObj.put("routes", jsonList);
  ...
  public void addRoute(final String route,
                       final AccessRequestHandler accessCallback,
    ...
    Spark.get(fullRoute, "application/json", (req, res) -> {
      ...
      logger
          .debug("Session info: id {}, creationTime {}, isNew {}, lastAccessedTime {}, maxInactiveInt {}",
                 req.session().id(),
                 req.session().creationTime(),
                 req.session().isNew(),
                 req.session().lastAccessedTime(),
      ...
      Map<String, Object> inputData =
          extractInputDataMap(req.queryMap().toMap());
      String subscriptionId = getSubscriptionId(authHeader);
      String accessSessionId =
          createAccessSessionId(req.headers("AccessSessionId"),
                                getSubscriberId(authHeader),
                                req.ip(),
                                req.session().id(),
                                inputData);

      BigIotHttpResponse bigIotResponse = accessCallback
          .processRequestHandler(offeringDescription,
                                 inputData,
                                 subscriptionId,
                                 accessSessionId);

      res.type("application/json");
      res.status(new Integer(bigIotResponse.getStatus()));
      for (Entry<String, String> entry : bigIotResponse
          .getHeaders()
          .entrySet()) {
        res.header(entry.getKey(), entry.getValue());
      }

      return bigIotResponse.getBody();
  ...
  public void removeRoute(String route) {
    ...
    if (!routes.containsKey(route)) {
      logger
          .error("ERROR: Cannot remove route ({}) for an undeployed Offering",
                 route);
    }

    Spark.get(baseRoute + "/" + route, "application/json", (req, res) -> {
      logger
          .info("Request received for Offering that has been removed: {}, {}, {}",
                req.pathInfo(),
                req.ip(),
                req.queryParams());
  ...
  public static void enableCors(final String origin, final String methods,
  ...
  private boolean
      checkAccessToken(final boolean authorizationRequired,
    ...
    if ((authHeader != null) && (authHeader.startsWith("Bearer "))) {
      ...
      if (!AccessToken
          .validateOfferingAccessToken(token,
                                       offeringDescription
                                           .getMarketplaceClient()
                                           .getClientSecret(),
                                       offeringDescription.getId())) {
        return false;
      }
    } else {
  ...
  private String getSubscriptionId(final String authHeader) {
    ...
    if ((authHeader != null) && (authHeader.startsWith("Bearer "))) {
      subscriptionId = AccessToken
          .getSubscriptionId(authHeader.substring(authHeader.indexOf(" ") + 1));
    }
  ...
  private String getSubscriberId(final String authHeader) {
    ...
    String subscriberId = "";
    if ((authHeader != null) && (authHeader.startsWith("Bearer "))) {
      subscriberId = AccessToken
          .getOfferingAccessTokenInfo(authHeader
  ...
  private String createAccessSessionId(String accessSessionId,
                                       final String subscriberId,
                                       final String reqIpAddress,
                                       final String reqSessionId,
                                       final Map<String, Object> inputData) {

    if (accessSessionId == null) {
      // Get accessSessionId from Input Data. If not available, auto-generate a
      // unique ID
      // from IP address, subscriberId, inputData Hash, etc.
      if (inputData != null) {
        if ((!inputData.isEmpty())
            && inputData.containsKey("accessSessionId")) {
          return String.valueOf(inputData.get("accessSessionId"));
        } else {
          if (!subscriberId.isEmpty()) {
            return reqIpAddress + "_" + subscriberId + "_"
                   + inputData.toString().hashCode();
          } else {
            return reqIpAddress + "_" + reqSessionId + "_"
                   + inputData.toString().hashCode();
          }
        }
      } else {
  ...
  private File writeTempFile(InputStream is) {
    ...
    try {
      ...
      logger
          .error("IOException during creating temporary server certificate file!");
